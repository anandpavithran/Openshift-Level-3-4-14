...
ssh root@utility 'tail -f /var/log/openshift/infra.log | egrep -o "\{.*}$" | jq '. | select(.systemd.u.SYSLOG_IDENTIFIER=="sshd")| .hostname + " " + .message'
Execute in another terminal ssh core@master01.ocp4.example.com whoami
Ctrl+C IN utility Machine
ssh root@utility egrep -ho "\{.*}$" audit.log* | jq '. | select(."audit.linux".type == "USER_LOGIN")| ."@timestamp" + " " + .hostname + " " + .message'
Execute next command--->see the output--->CTLR+C
ssh root@utility 'tail -f /var/log/openshift/infra.log | egrep -o "\{.*}$" | jq '. | select(.kubernetes.namespace_name == "openshift-storage")| .kubernetes.pod_name + " " + .message''
...
ssh root@utility 'tail -f audit.log | egrep -o "\{.*}$" | jq -c '.|select(.user.username == "developer")| [.user.username, .annotations, .verb, .objectRef]''
Execute next command --->Execute [oc apply -f ~/DO380/labs/logging-forward/logger.yml] in another terminal-->CTRL+c
ssh root@utility 'tail -f /var/log/openshift/infra.log | egrep -o "\{.*}$" | jq -c '.| select(.kubernetes.event.involvedObject.namespace == "logger-app")| [.kubernetes.event.involvedObject.kind,.kubernetes.event.involvedObject.name, .message]''
ssh root@utility 'grep -m1 shop-web /var/log/openshift/apps.log | egrep -o "\{.*}" | jq'
